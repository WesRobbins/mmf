...
class Spec2(BaseModel):
    ...
    def _build_ocr_encoding(self):
        ...
        self.linear_onehot_to_mmt_in = nn.Linear(4, self.mmt_config.hidden_size)
        self.ocr_onehot_layernorm = nn.LayerNorm(self.mmt_config.hidden_size)
    ...
    def _forward_ocr_encoding(self, sample_list, fwd_results):
        ...
        ocr_fasttext = self.reorder(ocr_fasttext, onehot)
        assert ocr_fasttext.size(-1) == 300

        # OCR PHOC feature (604-dim)
        ocr_phoc = sample_list.context_feature_1
        ocr_phoc = F.normalize(ocr_phoc, dim=-1)
        ocr_phoc = self.reorder(ocr_phoc, onehot)
...
@registry.register_model("spec_captioner2")
class SpecCaptioner2(Spec2):
    def __init__(self, config):
        super().__init__(config)
